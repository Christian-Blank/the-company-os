...
class TestPreCommitHooks:
    ...
    def test_sync_main_success(self):
        ...
        with patch(
            "company_os.domains.rules_service.adapters.pre_commit.hooks.sync"
        ) as mock_sync:
    ...
    def test_validate_main_with_markdown_files(self):
        ...
        with patch.object(sys, "argv", ["validate_hook", "README.md", "docs/guide.md"]):
            with patch(
                "company_os.domains.rules_service.adapters.pre_commit.hooks.validate_command"
            ) as mock_validate:
    ...
    def test_bazel_build_hooks(self):
        ...
        result = subprocess.run(
            [
                "bazel",
                "build",
                "//company_os/domains/rules_service/adapters/pre_commit:rules_sync_hook",
            ],
            capture_output=True,
            text=True,
        )
        ...
        result = subprocess.run(
            [
                "bazel",
                "build",
                "//company_os/domains/rules_service/adapters/pre_commit:rules_validate_hook",
            ],
            capture_output=True,
            text=True,
        )
...
class TestMainScript:
    ...
    def test_main_sync_detection(self):
        ...
        with patch.object(sys, "argv", ["rules_sync_hook"]):
            with patch(
                "company_os.domains.rules_service.adapters.pre_commit.hooks.sync_main"
            ) as mock_sync:
                mock_sync.return_value = 0
                with pytest.raises(SystemExit) as exc_info:
                    exec(
                        open(
                            "company_os/domains/rules_service/adapters/pre_commit/hooks.py"
                        ).read()
                    )
    ...
    def test_main_validate_detection(self):
        ...
        with patch.object(sys, "argv", ["rules_validate_hook"]):
            with patch(
                "company_os.domains.rules_service.adapters.pre_commit.hooks.validate_main"
            ) as mock_validate:
