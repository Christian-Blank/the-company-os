...
class Violation(BaseModel):
    ...
    context: Optional[str] = Field(None, description="Surrounding context for the violation")
...
class ScanStats(BaseModel):
    ...
    high_severity_count: int = Field(0, description="Number of high severity violations")
    medium_severity_count: int = Field(0, description="Number of medium severity violations")
...
class RegistryDefinition(BaseModel):
    ...
    scan_patterns: Optional[List[str]] = Field(None, description="Regex patterns to scan for")
    scan_file_types: Optional[List[str]] = Field(None, description="File patterns to scan")
    forbidden_patterns: Optional[List[str]] = Field(None, description="Patterns that should not exist")
    auto_fix: bool = Field(False, description="Whether auto-fix is supported")

    # Additional fields for specific definition types
    validation_pattern: Optional[str] = Field(None, description="Pattern to validate source value")
    forbidden_files: Optional[List[str]] = Field(None, description="Files that must not exist")
    replacement_rules: Optional[List[Dict[str, Any]]] = Field(None, description="Auto-fix replacement rules")
...
class CheckerConfig(BaseModel):
    ...
    registry_path: str = Field(..., description="Path to the source truth registry file")
